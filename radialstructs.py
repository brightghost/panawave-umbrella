...
class PanawaveBitmap(PWCanvasObject):
    '''A PIL.Image compatible with the PWCavnas, and with the same interface as
    PanawavePolygon, optionally allowing manual definition of a centroid point.'''

    def __init__(self, image, size=None, centroid=None):
        log.debug("Initializing PanawaveBitmap from file '{0}'".format(image))
        self.centroid = centroid
        # The largest of height or width will be scaled to this value. Uses
        # same units as the PanawaveStruct. Bitmap will not be scaled if not
        # set.
        self.size = size
        self.angle = 0
        self.x, self.y = (0, 0)
        ...
        if self.centroid is None:
            ...
            self.centroid = (0,0)
        self.path = image
        self.rawimage = Image.open(image).convert("RGBA")
        self.image = None # will be generated by composite_image()
        self.tkimage = None
        # '''Note: When a PhotoImage object is garbage-collected by Python
        # (e.g. when you return from a function which stored an image in a
        # local variable), the image is cleared even if it’s being displayed by
        # a Tkinter widget.'''
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, self.__class__):
            s_essence, o_essence = ((o.centroid, o.size, o.angle, o.x, o.y, o.path) for o in (self, other))
            log.debug("Comparing two essences to determine equivalency of PWBitmap objects: {0} {1}".format(
                s_essence, o_essence))
            return s_essence == o_essence
        else:
            return False

    def __neq__(self, other):
        '''Override default NotEquals behavior.'''
        return not self == other

    def copy(self):
        '''Return a copy of this object.'''
        return PanawaveBitmap(self.path, self.size, self.centroid)

    def as_string(self):
        '''Return a string representation of the object. This can be used to set a PWFileChooser.'''
        return self.path

    def _rotate(self, angle):
        '''Update the image with a rotated version.'''
        self.angle = self.angle + angle

    def rotate(self,angle):
        '''Update the image with a rotated version, and regenerate bitmap.'''
        self._rotate(angle)
        # self.composite_img()

    def _rotate_about_origin(self, angle):
        '''Update the angle and position attributes but do not regenerate image.'''
        complex_angle = exp(radians(angle) * 1j)
        new_complex_pos = complex(self.x, self.y) * complex_angle
        self.x, self.y = new_complex_pos.real, new_complex_pos.imag
        self.rotate(angle)

    def rotate_about_origin(self, angle):
        '''Rotate about origin with corresponding rotation of bitmap. Unlike
        PanawavePolygon, PanawaveBitmap has a single location point defined by
        self.x, self.y; image rotation and centroid defintion are all internal
        to the bitmap and not affected by the global position.'''
        self._rotate_about_origin(angle)
        # self.composite_img()
    ...
    def draw(self, canvas, selected=False, tags=None):
        '''Composite our image and draw to the indicated canvas.'''
        self.selected = selected
        self.composite_img()
        canvas.create_image(self.x, self.y, image=self.tkimage, tags=tags)

    def composite_img(self):
        '''To ensure they are applied in the right order and subsequent
        operations do not degrade image quality, all rotation, translation, and
        selection operations must be applied in the proper sequence immediately
        before drawing the bitmap. Thus self.image should be considered
        immutable, and self.tkimage ephemeral.'''
        # 1. apply border 2. rotate 3. 2nd rotate pass from rotate_about_origin
        # 4. scale 5. create imagetk 6. plot according to translation and r_about_origin
        self.image = self.rawimage.copy()
        if self.selected:
            self.image = ImageOps.expand(self.image, border=3, fill="#4285F4")
        self.image = self.image.rotate(-self.angle, expand=True, resample=Image.BICUBIC)
        sf = self._scale_factor
        self.image = self.image.resize(
                (int(self.image.width * sf), int(self.image.height * sf)),
                resample=Image.BICUBIC)
        self.tkimage = ImageTk.PhotoImage(self.image)

    def _get_scale_factor(self):
        '''Returns a scale factor calculated from the size attribute and the
        dimensions of the rawimage. This factor is needed because we are going
        to scale the rotated image, which will have different dimensions,
        thought PIL.Image.scale requires width and height values. Once
        the unit system for PanawaveStruct is clarified, self.size will be
        specified in those units and must be converted to pixels here.'''
        # TODO convert size to pix
        if self.size is None:
            return 1
        h, w = self.rawimage.height, self.rawimage.width
        if h > w:
            len = h
        else:
            len = w
        scale = float(self.size) / float(len)
        log.debug("Calculated new scale factor of {0}".format(scale))
        return scale
...
class PanawavePolygon(PWCanvasObject):
    ...
    def __init__(self, point_list, centroid=None):
        '''point_list:      list of tuples representing at least three points

        centroid:           tuple representing the center point about which the
                            object rotates. If not explicitly defined, it will
                            be calculated automatically using an algorithm
                            which is not guaranteed to be particularly
                            accurate.'''
        log.debug("Initting PWPoly with arguments {0} {1}".format(point_list, centroid))
        self.centroid = centroid
    ...
    def __eq__(self, other):
        '''Override default Equals behavior.'''
        if isinstance(other, self.__class__):
            s_essence, o_essence = ((o.centroid, o.points) for o in (self, other))
            log.debug("Comparing two essences to determine equivalency of PWBitmap objects: {0} {1}".format(
                s_essence, o_essence))
            return s_essence == o_essence
        else:
    ...
    def as_string(self):
        '''return string representing the Polygon. This string is valid input
        for the PWBaseStickerController.'''
        string_rep = ""
        for p in self.points:
            string_rep += repr(p).replace(" ", "")
        return string_rep

    def copy(self):
        '''Return a copy of this object.'''
        return PanawavePolygon(self.points, self.centroid)
...
class StickerRing:
    ...
    def __init__(self, radius, count, offsetDegrees=0, master=None, scaler_list=[1],
        ...
        if geometry is not None:
            self.base_sticker = PanawavePolygon(geometry)
        self.radius = float(radius)
        self.count = int(count)
        self.master = master # The parent PanawaveStruct; referenced for base_sticker
        self.base_sticker = None
    ...
    def _initialize_geometry(self):
        ...
        for s in self._stepper(self.count, self.scaler_list):
            if isinstance(self.base_sticker, PanawaveBitmap):
                # we need a new object for each sticker
                # because we need to roate and translate each differently.
                p = self.base_sticker.copy()
            elif isinstance(self.base_sticker, PanawavePolygon):
                p = self.base_sticker.copy()
            else:
                # Use the struct's base_sticker if none set here
                p = self.master.persistent_state['base_sticker'].copy()
    ...
    def reinherit_base_sticker(self):
        '''Re-apply inherited base_sticker if this ring's base_sticker is not
        explicitly set. This must be called whenever *PanawaveStruct's*
        base_sticker changes.'''
        self._initialize_geometry()

    def set_base_sticker(self, bmp_or_poly):
        '''Change the baseStickerPoly for this ring. Valid input:
        - A PanawavePolygon defining an enclosed polygon;
        - A PanawaveBitmap;
        - None: this ring will be configured to inherit the PanawaveStruct's base_sticker.'''
        if (isinstance(bmp_or_poly, PanawaveBitmap) or isinstance(bmp_or_poly, PanawavePolygon)):
            log.info("Resetting base_sticker for ring {0} to {1}".format(self.id, bmp_or_poly))
        elif bmp_or_poly is None:
            log.info("Clearing base_sticker for ring {0}.".format(self.id))
        else:
            raise TypeError
        self.base_sticker = bmp_or_poly
        self.reinherit_base_sticker()
...
class PanawaveStruct:
    ...
    def __init__(self, tkinstance=None, canvas=None, *args):
        ...
        self.persistent_state = {
            ...
            # of len(.scaler_list)
            "unlocked_rings": [],
            "base_sticker": PanawavePolygon([(0, 0), (0, 20), (20, 20), (20, 0)])
            }
    ...
    def add_ring(self, *args, **kwargs):
        ...
        #     kwargs['master'] = self
        kwargs['master'] = self
        self.canvas.delete("all") #TODO why are we deleting here?

        # We need to initialize the new ring before adding it to the ring_array
        # so we can reference it's id as the key.
        log.debug("all args: {0} {1}".format(evaluated_args, kwargs))
        new_ring = StickerRing(*evaluated_args, **kwargs)
        self.ring_array[str(new_ring.id)] = new_ring

    def set_base_sticker(self, bmp_or_poly=None):
        '''Provide a PanawavePolygon or PanawaveBitmap object to set new
        base_sticker. This sticker will be used as the default for new rings
        but can be overridden on a per-ring basis.'''
        # TODO rationalise this with StickerRing.set_base_sticker interface
        # The other interface with separate vars for poly and bitmap may be
        # better because we can retain the previous setting if the user wants to switch back
        log.info("Replacing base_sticker with the object {0}.".format(repr(bmp_or_poly)))
        self.persistent_state['base_sticker'] = bmp_or_poly
        # We also need to re-propogate to any existing rings inheriting from the base_sticker so they are ready to re-draw...
        for ring in self.ring_array.values():
            ring.reinherit_base_sticker()
